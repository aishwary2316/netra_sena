...
class ApiService {
  ...
  // Network helpers
  // change signature to accept optional headers
  Future<http.Response> _postJson(Uri uri, Object body, {Map<String, String>? headers}) async {
    _log('POST JSON -> $uri');
    try {
      // merge default json headers with any provided
      final merged = Map<String, String>.from(_jsonHeaders());
  ...
  // createUser now adds auth headers
  Future<Map<String, dynamic>> createUser({required String username, required String password, required String role}) async {
    ...
    final resp = await _postJson(uri, {'username': username, 'password': password, 'role': role}, headers: auth);
    if (resp.statusCode == 200 || resp.statusCode == 201) return jsonDecode(resp.body) as Map<String, dynamic>;
  ...
  Future<Map<String, dynamic>> deleteUserById(String id) async {
    ...
    if (resp.statusCode == 200 || resp.statusCode == 204) {
      if (resp.body.isEmpty || resp.body == 'null') {
        if (debug) print('ApiService.deleteUserById: empty body, returning success for id=$id');
        return {'success': true, 'message': 'User deleted successfully'};
      }
      final decoded = jsonDecode(resp.body) as Map<String, dynamic>;
      if (debug) print('ApiService.deleteUserById: decoded=$decoded');
      return decoded;
    }
